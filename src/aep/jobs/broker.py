...
def merge_preset_data_for_job(
    base_preset: dict,
    preset_overrides: dict | None,
    *,
    settings_decode_hwaccel: str,
) -> dict:
    """Deep-merge job overrides, then apply global decode hwaccel only when still ``auto``.

    Presets may pin ``decode.hwaccel`` (e.g. ``cuda``). The app settings value is a
    default for ``auto`` only; it must not replace an explicit preset or per-job
    override choice.
    """
    resolved = _deep_merge(dict(base_preset), dict(preset_overrides or {}))
    decode_block = resolved.setdefault("decode", {})
    if str(decode_block.get("hwaccel", "auto")).lower() == "auto":
        decode_block["hwaccel"] = settings_decode_hwaccel
    return resolved
...
class JobBroker:
    ...
    def _run_one(self, job: Job) -> None:
        ...
        # would silently use the on-disk preset). Global ``decode_hwaccel`` in app
        # settings applies only when the merged preset still has ``hwaccel: auto``.
        settings = load_settings()
        preset = load_preset(job.preset_id)
        resolved_preset_data = merge_preset_data_for_job(
            preset.model_dump(mode="json"),
            job.preset_overrides,
