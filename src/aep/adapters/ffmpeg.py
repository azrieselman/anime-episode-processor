...
class FFmpegAdapter(ToolAdapter):
    ...
    def build_decode_to_frames(
        ...
        *,
        ...
        png_compression: int | None = None,  # libpng branch only; defaults to constants.PNG_COMPRESSION_LEVEL
        png_intermediate_codec: PngIntermediateCodec = "mjpeg",
        ...
        When ``frame_format`` is ``"png"``, files use a ``.png`` suffix for pipeline
        compatibility. ``png_intermediate_codec`` chooses the payload: ``mjpeg``
        uses ``-c:v mjpeg -qmin 1 -qmax 1 -pix_fmt yuvj444p``; ``libpng`` uses
        true PNG with ``-compression_level`` from ``png_compression`` (defaulting
        to ``PNG_COMPRESSION_LEVEL``).
        """
        if frame_format not in ("png", "webp"):
            raise ValueError(f"frame_format must be png or webp, got {frame_format!r}")

        png_mode: PngIntermediateCodec
        if (png_intermediate_codec or "mjpeg").lower() == "libpng":
            png_mode = "libpng"
        else:
            png_mode = "mjpeg"
    ...
    def build_decode_to_frames_with_scene_metadata_fused(
        ...
        *,
        ...
        Stage 04 uses this for software decode only; with ``-hwaccel`` decode, a separate
        scene scan pass avoids sharing one filter graph with frame output (often faster).

        If ffmpeg rejects the graph, the caller falls back to decode + scan separately.

        PNG branch matches ``build_decode_to_frames`` (``png_intermediate_codec``).
        """
        if frame_format not in ("png", "webp"):
            raise ValueError(f"frame_format must be png or webp, got {frame_format!r}")
        ...
        # filter string. Mapping the raw input as well can attach an extra unfiltered
        # stream to the first image2 output (duplicate ``%08d`` files per frame, often
...
def decode_hwaccel_uses_hardware_decode(decode_hwaccel: str) -> bool:
    """True when decode uses ``-hwaccel`` (see ``_decode_input_args``)."""
