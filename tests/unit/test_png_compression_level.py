...
true libpng frames through ffmpeg: postprocess (s07), decode when
``png_intermediate_codec`` is libpng, and — should the encoder re-emit PNGs —
build_encode_from_frames.

Decode defaults to MJPEG into ``.png`` names when ``png_intermediate_codec`` is
mjpeg; see ``test_decode_to_frames_png_uses_mjpeg``.
...
def test_decode_to_frames_png_uses_mjpeg(tmp_path: Path) -> None:
    """PNG intermediate uses MJPEG + yuvj444p at max JPEG quality; output pattern stays .png."""
    ff = FFmpegAdapter()
    ...
    assert "-c:v" in cmd
    i = cmd.index("-c:v")
    assert cmd[i + 1] == "mjpeg"
    assert "-qmin" in cmd and cmd[cmd.index("-qmin") + 1] == "1"
    assert "-qmax" in cmd and cmd[cmd.index("-qmax") + 1] == "1"
    assert "-pix_fmt" in cmd and cmd[cmd.index("-pix_fmt") + 1] == "yuvj444p"
    assert "-compression_level" not in cmd
    assert cmd[-1].endswith("%08d.png")


def test_decode_mjpeg_branch_ignores_png_compression(tmp_path: Path) -> None:
    """MJPEG decode path does not pass libpng -compression_level."""
    ff = FFmpegAdapter()
    cmd = [str(c) for c in ff.build_decode_to_frames(
        source=tmp_path / "in.mkv",
        out_dir=tmp_path / "out",
...
def test_decode_libpng_branch_uses_compression_level(tmp_path: Path) -> None:
    ff = FFmpegAdapter()
    cmd = [str(c) for c in ff.build_decode_to_frames(
        source=tmp_path / "in.mkv",
        out_dir=tmp_path / "out",
        frame_format="png",
        png_intermediate_codec="libpng",
    )]
...
def test_decode_libpng_explicit_compression(tmp_path: Path) -> None:
    ff = FFmpegAdapter()
    cmd = [str(c) for c in ff.build_decode_to_frames(
        source=tmp_path / "in.mkv",
        out_dir=tmp_path / "out",
...
def test_fused_decode_emits_filter_complex_and_dual_maps(tmp_path: Path) -> None:
    ...
    assert cmd[-3:] == ["-f", "null", "-"]
    enc_map = cmd.index("-map")
    assert cmd[enc_map + 1] == "[enc]"
    enc_idx = cmd.index("-c:v", enc_map)
    assert cmd[enc_idx + 1] == "mjpeg"
    assert cmd[cmd.index("-qmin") + 1] == "1"
    assert cmd[cmd.index("-qmax") + 1] == "1"
    assert "-pix_fmt" in cmd and cmd[cmd.index("-pix_fmt") + 1] == "yuvj444p"
    assert any(s.endswith("%08d.png") for s in cmd)
    # Raw ``-map 0:v:0`` before filter_complex caused duplicate image2 outputs on some FFmpeg.
    i_fc = cmd.index("-filter_complex")
    assert not any(
        cmd[i] == "-map" and i + 1 < len(cmd) and cmd[i + 1] == "0:v:0" and i < i_fc
...
def test_fused_decode_libpng_branch(tmp_path: Path) -> None:
    from aep.util.frame_dedupe import SCENE_SCORE_META_BASENAME
