...
def _gui(category: str, tier: Literal["simple", "advanced"]) -> dict[str, object]:
    """Metadata consumed by the schema-driven preset editor."""
    return {"gui": {"category": category, "tier": tier}}
...
class UpscalerCfg(BaseModel):
    ...
    intermediate_format: Literal["png", "webp"] = Field(
        default="png",
        description="Lossless frame format between decode, upscale, interpolate, and encode; affects disk cache size.",
        json_schema_extra=_gui("upscaler", "advanced"),
    )
