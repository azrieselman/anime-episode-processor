...
def _rehydrate_plan_from_cached_stage_dir(
    ...
    try:
        if stage_name == "04_decode_serve":
            ...
            pic = params.get("png_intermediate_codec")
            if isinstance(pic, str) and pic:
                ctx.plan["decode"]["png_intermediate_codec"] = pic
            elif isinstance(metrics.get("png_intermediate_codec"), str):
                ctx.plan["decode"]["png_intermediate_codec"] = metrics["png_intermediate_codec"]
