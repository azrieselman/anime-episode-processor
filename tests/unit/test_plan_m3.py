...
def test_decode_plan_png_intermediate_codec_default() -> None:
    ...
    preset = _make_preset(upscaler_enabled=False, interp_enabled=False, pp_enabled=True, pp_deband=True)
    plan, _w, _r = _plan_m3_video_path(preset, media, primary)
    assert plan["decode"]["png_intermediate_codec"] == "mjpeg"


def test_decode_plan_png_intermediate_codec_from_preset() -> None:
    media, primary = _make_media()
    preset = _make_preset(upscaler_enabled=False, interp_enabled=False, pp_enabled=True, pp_deband=True)
    preset.decode = DecodeCfg(hwaccel="off", png_intermediate_codec="libpng")
    plan, _w, _r = _plan_m3_video_path(preset, media, primary)
    assert plan["decode"]["png_intermediate_codec"] == "libpng"
