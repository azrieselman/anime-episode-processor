...
class _FakeAnime4kLegacyAdapter:
    version = "2.5.0"
...
def test_stage05_dispatches_anime4k_legacy_engine(tmp_path: Path) -> None:
    workdir = tmp_path / "work"
    decode_dir = workdir / "04_decode_serve" / "frames"
    decode_dir.mkdir(parents=True, exist_ok=True)
    (decode_dir / "00000001.png").write_bytes(b"png")
    ...
    stage = UpscaleStage(anime4kcpp_legacy=_FakeAnime4kLegacyAdapter())  # type: ignore[arg-type]
    plan = stage.plan(ctx)
    result = stage.run(ctx, plan, EventSink())

    assert result.success is True
    assert result.metrics["engine"] == "anime4k-legacy"
    assert ctx.plan["upscale"]["count"] == 1
