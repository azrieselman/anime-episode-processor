...
# PNG compression level used for ALL intermediate PNG frames (decode, upscale,
# interpolate, postprocess). zlib level 0..9; 6 is zlib's default and the
# explicitly chosen sweet spot for AEP's frame-server pipeline:
...
# Wired into ffmpeg's PNG encoder (-compression_level) and ncnn binaries' -f
# flag (rife/realcugan/realesrgan/waifu2x all accept the same 0..9 mapping).
